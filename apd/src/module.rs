...
fn safe_module_dir(base: &Path, module_id: &str) -> Result<PathBuf> {
    ensure!(is_valid_module_id(module_id), "Invalid module id");
...
fn extract_archive_safely(archive: &mut zip::ZipArchive<fs::File>, destination: &Path) -> Result<()> {
    ...
    for index in 0..archive.len() {
        ...
        let enclosed_name = entry
            .enclosed_name()
            .ok_or_else(|| anyhow!("Invalid zip entry path: {}", entry.name()))?
            .to_owned();
        let output_path = destination.join(&enclosed_name);

        let parent = output_path
            .parent()
            .ok_or_else(|| anyhow!("Zip entry without parent: {}", entry.name()))?;
        fs::create_dir_all(parent)?;
        let canonical_parent = parent.canonicalize()?;
        ensure!(
            canonical_parent.starts_with(&destination),
            "Zip entry escaped destination: {}",
            entry.name()
        );
...
pub fn exec_script<T: AsRef<Path>>(path: T, wait: bool) -> Result<()> {
    ...
    command = command
        ...
        // Stealth: KSU-compatible env for module scripts
        .env("KSU", "true")
...
fn _install_module(zip: &str) -> Result<()> {
    ...
    if !module_dir.exists() {
        ...
        let permissions = fs::Permissions::from_mode(0o700);
        fs::set_permissions(&module_dir, permissions).expect("Failed to set permissions");
    }
    if module_update_tmp.exists() {
        remove_dir_all(&module_update_tmp)?;
    }
    if module_update_dir.exists() {
        remove_dir_all(&module_update_dir)?;
    }
    ...
    let module_system_dir = module_update_dir.join("system");
    ...
    if is_metamodule {
        ...
        metamodule::ensure_symlink(&module_dir.display().to_string())?;
    }
...
pub fn _uninstall_module(id: &str, update_dir: &str) -> Result<()> {
    ensure!(is_valid_module_id(id), "Invalid module id");
...
fn _change_module_state(module_dir: &str, mid: &str, enable: bool) -> Result<()> {
    ensure!(is_valid_module_id(mid), "Invalid module id");
