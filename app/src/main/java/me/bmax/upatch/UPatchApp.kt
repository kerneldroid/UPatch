...
import me.bmax.upatch.ui.CrashHandleActivity
import me.bmax.upatch.util.UPatchCli
import me.bmax.upatch.util.Version
import me.bmax.upatch.util.getRootShell
import me.bmax.upatch.util.rootShellForResult
import me.bmax.upatch.util.shQuote
import me.bmax.upatch.util.verifyAppSignature
...
lateinit var upApp: UPApplication

const val TAG = "App"

class UPApplication : Application(), Thread.UncaughtExceptionHandler {
    ...
    companion object {
        ...
        const val UPATCH_FOLDER = "/data/adb/up/"
        private const val APATCH_BIN_FOLDER = UPATCH_FOLDER + "bin/"
        private const val APATCH_LOG_FOLDER = UPATCH_FOLDER + "log/"
        private const val APD_LINK_PATH = APATCH_BIN_FOLDER + "apd"
        const val PACKAGE_CONFIG_FILE = UPATCH_FOLDER + "package_config"
        const val SU_PATH_FILE = UPATCH_FOLDER + "su_path"
        ...
        const val KPMS_DIR = UPATCH_FOLDER + "kpms/"

        @Deprecated("Use 'apd -V'")
        const val APATCH_VERSION_PATH = UPATCH_FOLDER + "version"
        ...
        fun uninstallApatch() {
            ...
            val cmds = arrayOf(
                "rm -f ${shQuote(APD_PATH)}",
                "rm -f ${shQuote(KPATCH_PATH)}",
                "rm -rf ${shQuote(APATCH_BIN_FOLDER)}",
                "rm -rf ${shQuote(APATCH_LOG_FOLDER)}",
                "rm -f ${shQuote(APATCH_VERSION_PATH)}",
            )

            val shell = getRootShell()
            val result = shell.newJob().add(*cmds).to(logCallback, logCallback).exec()
            if (!result.isSuccess) {
                Log.e(TAG, "UPatch uninstall failed: ${result.err.joinToString("\n")}")
                _apStateLiveData.postValue(State.ANDROIDPATCH_INSTALLED)
                return
            }

            UPatchCli.refresh()
            Log.d(TAG, "UPatch uninstalled...")
        ...
        fun installApatch() {
            ...
            val nativeDir = upApp.applicationInfo.nativeLibraryDir
            val previousState = state

            Natives.resetSuPath(LEGACY_SU_PATH)

            val cmds = arrayOf(
                "mkdir -p ${shQuote(APATCH_BIN_FOLDER)}",
                "mkdir -p ${shQuote(APATCH_LOG_FOLDER)}",

                "cp -f ${shQuote("${nativeDir}/libapd.so")} ${shQuote(APD_PATH)}",
                "chmod +x ${shQuote(APD_PATH)}",
                "rm -f ${shQuote(APD_LINK_PATH)}",
                "ln -sf ${shQuote(APD_PATH)} ${shQuote(APD_LINK_PATH)}",
                "restorecon ${shQuote(APD_PATH)}",

                "rm -f ${shQuote(MAGISKPOLICY_BIN_PATH)}",
                "ln -sf ${shQuote(APD_PATH)} ${shQuote(MAGISKPOLICY_BIN_PATH)}",
                "rm -f ${shQuote(RESETPROP_BIN_PATH)}",
                "ln -sf ${shQuote(APD_PATH)} ${shQuote(RESETPROP_BIN_PATH)}",

                "cp -f ${shQuote("${nativeDir}/libbusybox.so")} ${shQuote(BUSYBOX_BIN_PATH)}",
                "chmod +x ${shQuote(BUSYBOX_BIN_PATH)}",
                "cp -f ${shQuote("${nativeDir}/libkptools.so")} ${shQuote(KPTOOLS_BIN_PATH)}",
                "chmod +x ${shQuote(KPTOOLS_BIN_PATH)}",

                "touch ${shQuote(PACKAGE_CONFIG_FILE)}",
                "touch ${shQuote(SU_PATH_FILE)}",
                "[ -s ${shQuote(SU_PATH_FILE)} ] || printf %s ${shQuote(LEGACY_SU_PATH)} > ${shQuote(SU_PATH_FILE)}",
                "printf %s ${shQuote(Version.getManagerVersion().second.toString())} > ${shQuote(APATCH_VERSION_PATH)}",
                "restorecon -R ${shQuote(UPATCH_FOLDER)}",

                "${shQuote(MAGISKPOLICY_BIN_PATH)} --magisk --live",
            )

            val shell = getRootShell()
            val result = shell.newJob().add(*cmds).to(logCallback, logCallback).exec()
            if (!result.isSuccess) {
                Log.e(TAG, "UPatch install failed: ${result.err.joinToString("\n")}")
            ...
            Log.d(TAG, "UPatch installed...")
            _apStateLiveData.postValue(State.ANDROIDPATCH_INSTALLED)
        }

        fun markNeedReboot() {
            val result = rootShellForResult("touch ${shQuote(NEED_REBOOT_FILE)}")
            if (result.isSuccess) {
                _kpStateLiveData.postValue(State.KERNELPATCH_NEED_REBOOT)
            } else {
                Log.e(TAG, "mark reboot failed: ${result.err.joinToString("\n")}")
            }
    ...
    override fun onCreate() {
        ...
        if (!BuildConfig.DEBUG && BuildConfig.ENFORCE_SIGNATURE_VERIFICATION) {
            val expectedSignature = BuildConfig.EXPECTED_SIGNATURE_SHA256
            if (expectedSignature.isBlank()) {
                Log.w(TAG, "Signature verification is enabled but EXPECTED_SIGNATURE_SHA256 is empty.")
            } else if (!verifyAppSignature(expectedSignature)) {
                Toast.makeText(
                    applicationContext,
                    "${BuildConfig.APP_NAME}: signature verification failed.",
                    Toast.LENGTH_LONG
                ).show()
                exitProcess(0)
            }
        ...
        me.bmax.upatch.util.UPatchKeyHelper.setSharedPreferences(sharedPreferences)
        val savedKey = me.bmax.upatch.util.UPatchKeyHelper.readSPSuperKey()
                ...
                .addInterceptor { block ->
                    block.proceed(
                        block.request().newBuilder()
                            .header("User-Agent", "Android/${android.os.Build.VERSION.RELEASE}")
