...
object Natives {
    ...
    data class Profile(
        var uid: Int = 0,
        var toUid: Int = 0,
        var scontext: String = UPApplication.DEFAULT_SCONTEXT,
    ) : Parcelable
    ...
    fun su(toUid: Int, scontext: String?): Boolean {
        return nativeSu(UPApplication.superKey, toUid, scontext) == 0L
    }
    ...
    fun suPath(): String {
        return nativeSuPath(UPApplication.superKey)
    }
    ...
    fun loadKernelPatchModule(modulePath: String, args: String): Long {
        return nativeLoadKernelPatchModule(UPApplication.superKey, modulePath, args)
    }
    ...
    fun unloadKernelPatchModule(moduleName: String): Long {
        return nativeUnloadKernelPatchModule(UPApplication.superKey, moduleName)
    }
    ...
    fun kernelPatchModuleNum(): Long {
        return nativeKernelPatchModuleNum(UPApplication.superKey)
    }

    @FastNative
    private external fun nativeKernelPatchModuleList(superKey: String): String
    fun kernelPatchModuleList(): String {
        return nativeKernelPatchModuleList(UPApplication.superKey)
    }
    ...
    fun kernelPatchModuleInfo(moduleName: String): String {
        return nativeKernelPatchModuleInfo(UPApplication.superKey, moduleName)
    }
    ...
    fun kernelPatchModuleControl(moduleName: String, controlArg: String): KPMCtlRes {
        return nativeControlKernelPatchModule(UPApplication.superKey, moduleName, controlArg)
    }
    ...
    fun grantSu(uid: Int, toUid: Int, scontext: String?): Long {
        return nativeGrantSu(UPApplication.superKey, uid, toUid, scontext)
    }

    @FastNative
    private external fun nativeRevokeSu(superKey: String, uid: Int): Long
    fun revokeSu(uid: Int): Long {
        return nativeRevokeSu(UPApplication.superKey, uid)
    }
