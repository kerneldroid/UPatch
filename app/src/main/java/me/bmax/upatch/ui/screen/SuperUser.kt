...
import me.bmax.upatch.ui.viewmodel.SuperUserViewModel
import me.bmax.upatch.util.PkgConfig
import me.bmax.upatch.util.isValidSContext
    ...
    Scaffold(
        ...
        PullToRefreshBox(
            ...
            LazyColumn(Modifier.fillMaxSize()) {
                items(viewModel.appList.filter { it.packageName != upApp.packageName }, key = { it.packageName + it.uid }) { app ->
                    AppItem(app)
                }
            }
        }
        ...
        trailingContent = {
            Switch(checked = rootGranted, onCheckedChange = {
                ...
                if (rootGranted) {
                    ...
                    config.allow = 1
                    config.exclude = 0
                    config.profile.scontext = UPApplication.MAGISK_SCONTEXT
                } else {
                    config.allow = 0
                }
        ...
        SwitchItem(
            ...
            onCheckedChange = {
                if (it) {
                    excludeApp = 1
                    config.allow = 0
                    config.profile.scontext = UPApplication.DEFAULT_SCONTEXT
                    Natives.revokeSu(app.uid)
                } else {
                    excludeApp = 0
                }
