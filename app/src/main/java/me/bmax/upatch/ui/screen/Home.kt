package me.bmax.upatch.ui.screen
...
import me.bmax.upatch.R
import me.bmax.upatch.upApp
import me.bmax.upatch.ui.component.ProvideMenuShape
import me.bmax.upatch.ui.component.WarningCard
import me.bmax.upatch.ui.component.rememberConfirmDialog
import me.bmax.upatch.ui.viewmodel.PatchesViewModel
import me.bmax.upatch.util.LatestVersionInfo
import me.bmax.upatch.util.Version
import me.bmax.upatch.util.Version.getManagerVersion
import me.bmax.upatch.util.checkNewVersion
import me.bmax.upatch.util.getSELinuxStatus
import me.bmax.upatch.util.reboot
import me.bmax.upatch.util.ui.APDialogBlurBehindUtils
...
fun HomeScreen(navigator: DestinationsNavigator) {
    val kpState by UPApplication.kpStateLiveData.observeAsState(UPApplication.State.UNKNOWN_STATE)
    val apState by UPApplication.apStateLiveData.observeAsState(UPApplication.State.UNKNOWN_STATE)
    ...
    Scaffold(topBar = {
        ...
        Column(
            ...
            if (kpState != UPApplication.State.UNKNOWN_STATE && apState != UPApplication.State.ANDROIDPATCH_INSTALLED) {
                AStatusCard(apState)
            }
            val checkUpdate = UPApplication.sharedPreferences.getBoolean("check_update", true)
            if (checkUpdate) {
                UpdateCard()
            }
...
@Composable
private fun TopBar(
    onInstallClick: () -> Unit, navigator: DestinationsNavigator, kpState: UPApplication.State
) {
    ...
    TopAppBar(title = {
        ...
        if (kpState != UPApplication.State.UNKNOWN_STATE) {
            IconButton(onClick = {
                ...
                ProvideMenuShape(RoundedCornerShape(10.dp)) {
                    DropdownMenu(expanded = showDropdownMoreOptions, onDismissRequest = {
                        ...
                        DropdownMenuItem(text = {
                            ...
                            showDropdownMoreOptions = false
                            uriHandler.openUri(BuildConfig.ISSUES_URL)
                        })
                        DropdownMenuItem(text = {
...
private fun KStatusCard(
    kpState: UPApplication.State, apState: UPApplication.State, navigator: DestinationsNavigator
) {
    ...
    val cardBackgroundColor = when (kpState) {
        UPApplication.State.KERNELPATCH_INSTALLED -> {
            MaterialTheme.colorScheme.primary
        }

        UPApplication.State.KERNELPATCH_NEED_UPDATE, UPApplication.State.KERNELPATCH_NEED_REBOOT -> {
    ...
    ElevatedCard(
        onClick = {
            if (kpState != UPApplication.State.KERNELPATCH_INSTALLED) {
                navigator.navigate(InstallModeSelectScreenDestination)
            }
        },
        colors = CardDefaults.elevatedCardColors(containerColor = cardBackgroundColor),
        elevation = CardDefaults.cardElevation(
            defaultElevation = if (kpState == UPApplication.State.UNKNOWN_STATE) 0.dp else 6.dp
        )
    ) {
        Column(
            modifier = Modifier
            ...
            if (kpState == UPApplication.State.KERNELPATCH_NEED_UPDATE) {
                Row {
                    Text(
            ...
            Row(
                ...
                Column(
                    ...
                    if (kpState != UPApplication.State.UNKNOWN_STATE && kpState != UPApplication.State.KERNELPATCH_NEED_UPDATE && kpState != UPApplication.State.KERNELPATCH_NEED_REBOOT) {
                        Spacer(Modifier.height(4.dp))
                        Text(
                            text = "${Version.installedKPVString()} (${managerVersion.second}) - " + if (apState != UPApplication.State.ANDROIDPATCH_NOT_INSTALLED) "Full" else "KernelPatch",
                            style = MaterialTheme.typography.bodyMedium
                        )
                    }
                }
                ...
                Column(
                    ...
                    Button(onClick = {
                        when (kpState) {
                            UPApplication.State.UNKNOWN_STATE -> {
                                navigator.navigate(InstallModeSelectScreenDestination)
                            }

                            UPApplication.State.KERNELPATCH_NEED_UPDATE -> {
                                navigator.navigate(InstallModeSelectScreenDestination)
                            }

                            UPApplication.State.KERNELPATCH_NEED_REBOOT -> {
                                reboot()
                            }

                            UPApplication.State.KERNELPATCH_UNINSTALLING -> {
                                // Do nothing
                            }

                            else -> {
                                if (apState == UPApplication.State.ANDROIDPATCH_INSTALLED || apState == UPApplication.State.ANDROIDPATCH_NEED_UPDATE) {
...
@Composable
private fun AStatusCard(apState: UPApplication.State) {
    ElevatedCard(
        ...
        Column(
            ...
            Row(
                ...
                when (apState) {
                    UPApplication.State.ANDROIDPATCH_NOT_INSTALLED -> {
                        Icon(Icons.Outlined.Block, stringResource(R.string.home_not_installed))
                    }

                    UPApplication.State.ANDROIDPATCH_INSTALLING -> {
                        Icon(Icons.Outlined.InstallMobile, stringResource(R.string.home_installing))
                    }

                    UPApplication.State.ANDROIDPATCH_INSTALLED -> {
                        Icon(Icons.Outlined.CheckCircle, stringResource(R.string.home_working))
                    }

                    UPApplication.State.ANDROIDPATCH_NEED_UPDATE -> {
                        Icon(Icons.Outlined.SystemUpdate, stringResource(R.string.home_need_update))
                    }
                ...
                Column(
                    ...
                    when (apState) {
                        ...
                        UPApplication.State.ANDROIDPATCH_NEED_UPDATE -> {
                            ...
                            Text(
                                text = stringResource(
                                    R.string.upatch_version_update,
                ...
                if (apState != UPApplication.State.UNKNOWN_STATE) {
                    Column(
                        modifier = Modifier.align(Alignment.CenterVertically)
                    ) {
                        Button(onClick = {
                            when (apState) {
                                UPApplication.State.ANDROIDPATCH_NOT_INSTALLED, UPApplication.State.ANDROIDPATCH_NEED_UPDATE -> {
                                    UPApplication.installApatch()
                                }

                                UPApplication.State.ANDROIDPATCH_UNINSTALLING -> {
                                    // Do nothing
                                }
...
@Composable
fun WarningCard() {
    var show by rememberSaveable { mutableStateOf(upApp.getBackupWarningState()) }
    if (show) {
        ElevatedCard(
            elevation = CardDefaults.cardElevation(
                    ...
                    Row(
                        ...
                        Icon(
                            ...
                            modifier = Modifier.clickable {
                                show = false
                                upApp.updateBackupWarningState(false)
                            },
                        )
                    }
                }
...
@Composable
private fun InfoCard(kpState: UPApplication.State, apState: UPApplication.State) {
    ElevatedCard {
        Column(
            modifier = Modifier
                .fillMaxWidth()
            ...
            if (apState != UPApplication.State.UNKNOWN_STATE && apState != UPApplication.State.ANDROIDPATCH_NOT_INSTALLED) {
                InfoCardItem(
                    stringResource(R.string.home_upatch_version), managerVersion.second.toString()
                )
                Spacer(Modifier.height(16.dp))
            }
...
fun UpdateCard() {
    ...
    AnimatedVisibility(
        ...
        WarningCard(
            message = stringResource(id = R.string.home_new_upatch_found).format(newVersionCode),
    ...
    ElevatedCard {
        Row(
            modifier = Modifier
                .fillMaxWidth()
                .clickable {
                    uriHandler.openUri(BuildConfig.DOCS_URL)
                }
                .padding(24.dp), verticalAlignment = Alignment.CenterVertically) {
            Column {
                Text(
                    text = stringResource(R.string.home_learn_upatch),
                    style = MaterialTheme.typography.titleSmall
                )
                Spacer(Modifier.height(4.dp))
                Text(
                    text = stringResource(R.string.home_click_to_learn_upatch),
                    style = MaterialTheme.typography.bodyMedium
                )
            }
        }
...
fun AuthDialog(showDialog: MutableState<Boolean>) {
    var skey by remember { mutableStateOf(me.bmax.upatch.UPApplication.superKey) }
        ...
        Surface(
            ...
            Column(modifier = Modifier.padding(24.dp)) {
                ...
                Row(
                    ...
                    TextButton(onClick = {
                        me.bmax.upatch.util.UPatchKeyHelper.writeSPSuperKey(skey)
                        me.bmax.upatch.UPApplication.superKey = skey
                        me.bmax.upatch.util.UPatchCli.refresh()
                        showDialog.value = false
                    }) {
