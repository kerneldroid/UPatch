...
import me.bmax.upatch.ui.component.SwitchItem
import me.bmax.upatch.ui.component.rememberLoadingDialog
import me.bmax.upatch.ui.theme.refreshTheme
import me.bmax.upatch.util.getBugreportFile
import me.bmax.upatch.util.isGlobalNamespaceEnabled
import me.bmax.upatch.util.outputStream
import me.bmax.upatch.util.rootShellForResult
import me.bmax.upatch.util.shQuote
import me.bmax.upatch.util.setGlobalNamespaceEnabled
import me.bmax.upatch.util.ui.APDialogBlurBehindUtils
import me.bmax.upatch.util.ui.LocalSnackbarHost
import me.bmax.upatch.util.ui.NavigationBarsSpacer
...
fun SettingScreen() {
    ...
    val kPatchReady = state != UPApplication.State.UNKNOWN_STATE
    val aPatchReady =
        (state == UPApplication.State.ANDROIDPATCH_INSTALLING || state == UPApplication.State.ANDROIDPATCH_INSTALLED || state == UPApplication.State.ANDROIDPATCH_NEED_UPDATE)
    ...
    Scaffold(
        ...
        Column(
            ...
            val context = LocalContext.current
            val scope = rememberCoroutineScope()
            val prefs = UPApplication.sharedPreferences
            ...
            if (aPatchReady) {
                ...
                SwitchItem(
                    ...
                    UPApplication.sharedPreferences.edit {
                                    ...
                                    modifier = Modifier
                                        ...
                                        .clickable {
                                            scope.launch {
                                                ...
                                                exportBugreportLauncher.launch("UPatch_bugreport_${current}.tar.gz")
            ...
            Column(modifier = Modifier.padding(PaddingValues(all = 24.dp))) {
                ...
                Row(
                    ...
                    Button(enabled = suPathChecked(suPath), onClick = {
                        ...
                        if (success) {
                            rootShellForResult("printf %s ${shQuote(suPath)} > ${shQuote(UPApplication.SU_PATH_FILE)}")
                        }
                    }) {
