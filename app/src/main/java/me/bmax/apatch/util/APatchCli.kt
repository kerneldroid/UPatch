...
fun execApd(vararg args: String, newShell: Boolean = false): Boolean {
    val command = shCommand(listOf(APApplication.APD_PATH) + args.toList())
    return if (newShell) {
        withNewRootShell {
...
fun listModules(): String {
    val shell = getRootShell()
    val out = shell.newJob().add(shCommand(APApplication.APD_PATH, "module", "list")).to(ArrayList(), null).exec().out
...
fun installModule(
    ...
    val file = File.createTempFile("module_${type.name.lowercase()}_", ".zip", apApp.cacheDir)
    ...
    return try {
        ...
        val result = if (type == MODULE_TYPE.APM) {
            val cmd = shCommand(APApplication.APD_PATH, "module", "install", file.absolutePath)
...
fun runAPModuleAction(
    ...
    val result = withNewRootShell{ 
        newJob().add(shCommand(APApplication.APD_PATH, "module", "action", moduleId))
...
fun isGlobalNamespaceEnabled(): Boolean {
    val shell = getRootShell()
    val result = ShellUtils.fastCmd(shell, shCommand("cat", APApplication.GLOBAL_NAMESPACE_FILE))
...
fun setGlobalNamespaceEnabled(value: String) {
    ...
    getRootShell().newJob().add("printf %s ${shQuote(value)} > ${shQuote(APApplication.GLOBAL_NAMESPACE_FILE)}")
        .submit { result ->
